...
import Level from '../src/stores/level.js'
...
describe('stores', () => {
  for (const [name, Store] of Object.entries(stores)) {
    for (const cached of [true, false]) {
      const lruSize = name === 'level' ? false : 1024 * 1024 * 50
      const lru = cached ? { lruSize } : { lru: false }
      let create
      if (name === 's3') {
        create = q => Store.create(q, { s3: new MockS3(), ...lru })
      } else if (name === 'level') {
        create = q => Store.create(q, { leveldown: memdown(), ...lru })
      } else {
        create = q => Store.create(q, lru)
      }

      describe(`${name} ${!cached ? 'no lru' : ''}`, () => {
        let cid
        ...
        it('insert twice, missing column in second insert', async () => {
          let ipsql = await create('CREATE TABLE Test (ID int, String varchar(255))')
          ipsql = await ipsql.write('INSERT INTO Test VALUES ( 11, \'a\' )')
          // clear cache so that we test without LRU hits
          if (ipsql.lru) {
            ipsql.lru.reset()
          }
          ipsql = await ipsql.write('INSERT INTO Test (ID) VALUES ( 10 )')
